...
class Node(object):
    ...
    #Calculates the number of linear conflicts
    def calculateLinearConflict(self, goal):
        size = len(self.state)
        #Creating empty lists that will store the positions of the numbers
        inRow = [0] * (size**2)
        inCol = [0] * (size**2)
        cnt = 0
        
        #Store where each number is in goal state
        for i in range(size):
            for j in range(size):
                n = self.state[i][j]
                goalPosition = self.findGoalPosition(n, goal)
                inRow[n] = goalPosition[0]
                inCol[n] = goalPosition[1]
                
        #Check for row conflicts
        for r in range(size):
            for c1 in range(size):
                for c2 in range(c1 + 1, size):
                    if self.state[r][c1] == 0 or self.state[r][c2] == 0:
                        continue
                    #checking if they're both in the same row in goal and diff columns such that the value in c1 > the value
                    # in c2 even though c2 > c1. This means there is a linear conflict.
                    if r == inRow[self.state[r][c1]] and inRow[self.state[r][c1]] == inRow[self.state[r][c2]] and inCol[self.state[r][c1]] > inCol[self.state[r][c2]]:
                        cnt = cnt + 1
        ...
        for c in range(size):
            for r1 in range(size):
                for r2 in range(r1 + 1, size):
                    if self.state[r1][c] == 0 or self.state[r2][c] == 0:
                        continue
                    #checking if they're both in the same column in the goal and diff columns such that the value in r1 > the value
                    #in r2 even though r2 > r1. This means there is a linear conflict.
                    if c == inCol[self.state[r1][c]] and inCol[self.state[r1][c]] == inCol[self.state[r2][c]] and inRow[self.state[r1][c]] > inRow[self.state[r2][c]]:
                        cnt = cnt + 1
        return cnt
    ...
    def calculateH(self, goal):
        ...
        return total + 2 * self.calculateLinearConflict(goal)
